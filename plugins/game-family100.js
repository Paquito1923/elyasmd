...
async function handler(m) {
    this.game = let fetch = require('node-fetch')
let winScore = 999
let family100_api = [
...
async function handler(m) {
    ...
    if (id in this.game) {
        conn.sendBut(m.chat, 'Masih ada kuis yang belum terjawab di chat ini','', 'Nyerah', 'nyerah', this.game[id].msg)
        throw false
    }
...
module.exports = handler ? this.game : {}
    let id = 'family100_' + m.chat
    if (id in this.game) {
        conn.sendBut(m.chat, 'Masih ada kuis yang belum terjawab di chat ini', '', 'Nyerah', 'nyerah', this.game[id].msg)
        throw false
