  ...
  try {
    if (/craft|Crafting/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
          case 'pickaxe':
            if(user.kayu < 10 || user.iron < 5 || user.string < 20) return conn.send3But(m.chat,`Barang tidak cukup!\nUntuk membuat pickaxe. Kamu memerlukan : \n10 kayu🪵 \n5 iron⛓\n20 String🕸️`, 'ketik .shop untuk membeli/menjual barang', 'Beli 10 kayu🪵', '.shop buy kayu 10', 'beli 5 iron⛓', '.shop buy iron 5', 'beli 20 String🕸️', '.shop buy string 20', m)
